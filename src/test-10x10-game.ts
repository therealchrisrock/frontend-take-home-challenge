...
console.log(
  `Board: ${rules.board.size}×${rules.board.size} International Draughts\n`,
);
...
console.log(
  `Total valid moves for BLACK: ${blackMoves} (should be 0, red goes first)\n`,
);
...
if (testMoves.length > 0) {
  ...
  console.log(
    `- Moving from (${move.from.row},${move.from.col}) to (${move.to.row},${move.to.col})`,
  );
  ...
  console.log(
    `- Piece at destination: ${movedPiece ? `${movedPiece.color} ${movedPiece.type}` : "none"}`,
  );
  console.log(
    `- Piece at origin: ${oldPosition ? "ERROR - piece still there!" : "empty (correct)"}`,
  );
