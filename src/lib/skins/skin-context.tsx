...
interface SkinContextValue {
  ...
  checkUnlockCondition: (skinId: string, type: string, value: number | string) => void;
...
export function SkinProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (savedProgress) {
      try {
        const progress = JSON.parse(savedProgress) as Array<[string, SkinUnlockProgress]>;
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--piece-black-from', skin.pieces.black.gradient.from);
  ...
  const unlockSkin = useCallback((skinId: string) => {
    ...
    setUnlockedSkins(prev => {
      ...
      newSet.add(skinId);
      localStorage.setItem(STORAGE_KEY_UNLOCKED, JSON.stringify(Array.from(newSet)));
      return newSet;
    });
    ...
    setSkinProgress(prev => {
      ...
      localStorage.setItem(STORAGE_KEY_PROGRESS, JSON.stringify(Array.from(newMap)));
      return newMap;
    });
  }, []);

  const checkUnlockCondition = useCallback((skinId: string, type: string, value: number | string) => {
    ...
    if (condition.type === type) {
      if (condition.type === 'code' && condition.value === value) {
        unlockSkin(skinId);
      } else if (typeof condition.value === 'number' && typeof value === 'number' && value >= condition.value) {
        unlockSkin(skinId);
  ...
  const updateProgress = useCallback((skinId: string, progress: number) => {
    const skin = skins[skinId];
    if (!skin?.unlockCondition) return;
    if (unlockedSkins.has(skinId)) return;
