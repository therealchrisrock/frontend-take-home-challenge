...
export interface MoveEvaluation {
  ...
  alternativeMoves?: {            // Top alternative moves for comparison
    move: Move;
    score: number;
    notation: string;
  }[];
...
export interface GameAnalysis {
  ...
  evaluationGraph: {             // For visualization
    moveIndex: number;
...
export const MOVE_INDICATORS = {
  brilliant: { 
    ...
    color: 'cyan', 
    label: 'Brilliant!',
    description: 'A surprising, non-obvious move that significantly improves the position'
  },
  excellent: { 
    icon: '⭐', 
    color: 'gold', 
...
export function getPlayerAdvantage(evaluation: number): { player: PieceColor; advantage: number } {
  return {
