...
export interface DrawResult {
  type: 'draw';
  reason: 'threefold-repetition' | 'forty-move-rule' | 'twenty-five-move-rule' | 'insufficient-material' | 'stalemate';
  explanation: string;
}
 ...
 */
export function serializeBoard(board: Board, currentPlayer: PieceColor): string {
 ...
 */
export function checkThreefoldRepetition(state: DrawState, config: VariantConfig): boolean {
 ...
 */
export function checkFortyMoveRule(state: DrawState, config: VariantConfig): boolean {
  if (!config.draws.fortyMoveRule) return false;
  
  // In checkers, it's typically 40 moves by each player (80 plies)
  const moveLimit = 80;
  return state.movesSinceCapture >= moveLimit && state.movesSincePromotion >= moveLimit;
}
...
export function checkTwentyFiveMoveRule(
  ...
  let redRegular = 0, blackRegular = 0;
  ...
  const isKingEndgame = redRegular === 0 && blackRegular === 0 && redKings > 0 && blackKings > 0;
 ...
 */
export function checkInsufficientMaterial(board: Board, config: VariantConfig): boolean {
  ...
  if (redTotal === 1 && blackTotal === 1 && redKings === 1 && blackKings === 1) {
    return true;
  }
  
  // 2. Two kings vs one king (can't force a win in most variants)
  if ((redTotal === 2 && blackTotal === 1 && redKings === 2 && blackKings === 1) ||
  ...
  if (config.board.size >= 10) {
    if ((redTotal === 3 && blackTotal === 1 && redKings === 3 && blackKings === 1) ||
...
export function checkDrawConditions(
  ...
  if (checkThreefoldRepetition(state, config)) {
    return {
      ...
      explanation: 'The same position has occurred three times. This results in a draw by threefold repetition.'
    };
  }
  ...
  if (checkFortyMoveRule(state, config)) {
    return {
      ...
      explanation: 'Neither player has captured a piece or promoted a checker in the last 40 moves. This results in a draw by the forty-move rule.'
    };
  }
  ...
  if (checkTwentyFiveMoveRule(board, state, config)) {
    return {
      ...
      explanation: 'In this king-only endgame, no captures have occurred in the last 25 moves. This results in a draw by the twenty-five-move rule.'
    };
  }
  
  // Check insufficient material
  if (checkInsufficientMaterial(board, config)) {
    const redKings = board.flat().filter(p => p?.color === 'red' && p.type === 'king').length;
    const blackKings = board.flat().filter(p => p?.color === 'black' && p.type === 'king').length;
