...
class ConfigurableBoardValidator {
  ...
  isValidSquare(row: number, col: number): boolean {
    return row >= 0 && row < this.config.board.size && 
           col >= 0 && col < this.config.board.size;
  }
...
export class GameRules {
    ...
    for (const row of this.config.board.startingRows.black) {
      for (let col = 0; col < size; col++) {
        if (this.validator.isDarkSquare(row, col)) {
          this.setPiece(board, { row, col }, { color: 'black', type: 'regular' });
        }
      }
    }
   ...
   */
  getValidDirections(piece: { color: PieceColor; type: PieceType }): readonly [number, number][] {
   ...
   */
  shouldPromote(piece: { color: PieceColor; type: PieceType }, toRow: number): boolean {
   ...
   */
  private isSameDirection(dRow: number, dCol: number, validDRow: number, validDCol: number): boolean {
  ...
  private isMaximumCaptureMove(board: Board, move: Move): boolean {
    ...
    const maxCaptures = Math.max(...allCaptureMoves.map(m => m.captures?.length ?? 0));
   ...
   */
  private getAllCaptureMoves(board: Board, from: Position, piece: Piece): Move[] {
    const moves: Move[] = [];
    ...
    for (const [dRow, dCol] of directions) {
      if (piece.type === 'king' && this.canFlyAsKing()) {
        ...
        moves.push(...this.getFlyingKingCaptures(board, from, [dRow, dCol], piece));
      } else {
        // Regular captures (one square jump)
        moves.push(...this.getRegularCaptures(board, from, [dRow, dCol], piece));
      }
    }
   ...
   */
  private getFlyingKingCaptures(board: Board, from: Position, direction: [number, number], piece: Piece): Move[] {
    const moves: Move[] = [];
    ...
    while (true) {
      ...
      if (checkPiece) {
        ...
        const furtherCaptures = this.getAllCaptureMoves(tempBoard, { row: checkRow, col: checkCol }, piece);
   ...
   */
  private getRegularCaptures(board: Board, from: Position, direction: [number, number], piece: Piece): Move[] {
    const moves: Move[] = [];
    const [dRow, dCol] = direction;
    ...
    if (
      ...
      this.getPiece(board, { row: captureRow, col: captureCol })!.color !== piece.color &&
      ...
      // Look for further captures
      const furtherCaptures = this.getAllCaptureMoves(tempBoard, { row: landRow, col: landCol }, piece);
      ...
      for (let col = 0; col < size; col++) {
        ...
        if (piece && piece.color === player) {
          ...
          for (const [dRow, dCol] of directions) {
            ...
            if (this.validator.isValidSquare(newRow, newCol) && 
                !board[newRow]![newCol]) {
              const move: Move = {
                from: { row, col },
  ...
  checkWinner(board: Board): PieceColor | 'draw' | null {
    ...
    if (this.config.draws.insufficientMaterial && this.checkInsufficientMaterial(board)) {
      return 'draw';
    }
   ...
   */
  checkDrawCondition(board: Board, moveHistory: Move[], currentPlayer?: PieceColor): 'draw' | null {
