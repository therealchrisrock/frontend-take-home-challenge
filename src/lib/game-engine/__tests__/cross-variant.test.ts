...
describe('Cross-Variant Tests', () => {
    ...
    VARIANT_TEST_CASES.forEach(testCase => {
      describe(`${testCase.variant.toUpperCase()} variant`, () => {
        ...
        it('should enforce backward capture rules correctly', () => {
          expect(rules.canCaptureBackward({ color: 'red', type: 'regular' }))
            .toBe(testCase.rules.backwardCapture.regular);
          expect(rules.canCaptureBackward({ color: 'black', type: 'regular' }))
            .toBe(testCase.rules.backwardCapture.regular);
          expect(rules.canCaptureBackward({ color: 'red', type: 'king' }))
            .toBe(testCase.rules.backwardCapture.king);
        });
        ...
        it('should enforce capture priority rules', () => {
          expect(rules.isMandatoryCapture()).toBe(testCase.rules.mandatoryCapture);
          expect(rules.requiresMaximumCapture()).toBe(testCase.rules.maximumCapture);
          expect(rules.requiresKingPriority()).toBe(testCase.rules.kingPriority);
        });
        ...
        it('should handle promotion at correct rows', () => {
          ...
          expect(rules.shouldPromote({ color: 'black', type: 'regular' }, lastRow)).toBe(true);
          expect(rules.shouldPromote({ color: 'black', type: 'king' }, lastRow)).toBe(false);
        });
    ...
    VARIANTS.forEach(variant => {
      describe(`${variant} - Common behaviors`, () => {
        ...
        it('should correctly change piece type on promotion', () => {
          const size = rules.getBoardSize();
          const testBoard: Board = Array(size).fill(null).map(() => Array(size).fill(null));
  ...
  describe('Variant-Specific Capture Sequences', () => {
    ...
    it('International: should enforce maximum capture rule', async () => {
      ...
      const hasCaptures = captures.some(m => m.captures && m.captures.length > 0);
      if (hasCaptures) {
        const nonCaptures = captures.filter(m => !m.captures || m.captures.length === 0);
