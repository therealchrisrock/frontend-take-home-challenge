...
export function SendFriendRequestDialog({
    ...
    try {
      if (!selectedUser?.id) {
        toast({
          ...
          description: "You must select a user from the search results to send a friend request.",
          variant: "destructive",
        });
        return;
      }
  ...
  const getStatusBadge = () => {
    ...
    switch (requestStatus.status) {
      ...
      case "self":
        return <Badge variant="outline">This is you</Badge>;
      default:
        return null;
    }
  };

  const canSendRequest = requestStatus?.status === "none";
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            {requestStatus && !canSendRequest && (
              <div className="flex items-center gap-2 p-2 bg-yellow-50 border border-yellow-200 rounded-md">
                ...
                <p className="text-sm text-yellow-800">
                  {requestStatus.status === "friends" && "You are already friends with this user."}
                  ...
                  {requestStatus.status === "blocked" && "Unable to send friend request to this user."}
                  {requestStatus.status === "self" && "You cannot send a friend request to yourself."}
                </p>
              </div>
            )}
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button
                type="submit"
                disabled={sendMutation.isPending || !canSendRequest || !selectedUser}
              >
