...
export function ChatMessages({
  ...
  return (
    <div className={`h-64 space-y-2 overflow-y-auto p-3 ${getThemeClasses()}`}>
        ...
        if (isSystem) {
          return (
            <div key={message.id} className="text-center">
              <span
                className={`rounded px-2 py-1 text-xs opacity-75 ${theme === "dark" ? "bg-gray-800/50 text-gray-400" : "bg-gray-100 text-gray-600"} `}
              >
                {message.content}
              </span>
            </div>
            ...
            >
              {showAvatar && (
                <div className="mb-1 flex items-center gap-2">
                  {!isOwnMessage && (
                    <span
                      className={`text-xs font-medium ${theme === "dark" ? "text-gray-300" : "text-gray-700"} `}
                    >
                      {message.senderName}
                    </span>
                  )}
                  <span
                    className={`text-xs opacity-60 ${theme === "dark" ? "text-gray-400" : "text-gray-500"} `}
                  >
