...
export function ChatFriendsPopup({ isOpen, onClose, onOpenDM, theme }: ChatFriendsPopupProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={`max-w-2xl h-[600px] ${themeClasses}`}>
        <DialogHeader>
        ...
        <Tabs defaultValue="friends" className="flex-1 flex flex-col">
                  ...
                  `}>
                    ...
                    <div className="flex gap-2">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => onOpenDM(friend.id, friend.name ?? friend.username)}
            ...
            <div className="space-y-4">
              ...
              <ScrollArea className="h-[400px]">
                ...
                {searchQuery.length <= 2 && (
                  <div className={`text-center py-12 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                    <Search className="h-16 w-16 mx-auto mb-4 opacity-50" />
                    <p className="text-lg mb-2">Search for friends</p>
          ...
          {/* Blocked Users */}
          <TabsContent value="blocked" className="flex-1 mt-4">
            <ScrollArea className="h-[450px]">
              {blockedUsers?.length === 0 && (
                <div className={`text-center py-12 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                  <Shield className="h-16 w-16 mx-auto mb-4 opacity-50" />
                  <p className="text-lg mb-2">No blocked users</p>
                </div>
              )}
              <div className="space-y-3">
