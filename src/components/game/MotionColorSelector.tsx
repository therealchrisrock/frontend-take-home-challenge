 ...
 */
export function MotionColorSelector({ value, onChange }: MotionColorSelectorProps) {
  const [isHovering, setIsHovering] = useState<string | null>(null);

  const colors = [
    { id: 'red', label: 'Red', color: 'bg-red-600', border: 'border-red-700' },
    { id: 'black', label: 'Black', color: 'bg-gray-800', border: 'border-gray-900' },
  ...
  return (
    <div>
      <Label className="text-gray-700 text-sm mb-2 block">Your Color</Label>
      
      <LayoutGroup>
        <div className="grid grid-cols-3 gap-1 bg-gray-100 rounded-lg p-1" style={{ minHeight: '44px' }}>
          {/* Color options */}
          {colors.map((color) => (
            <div key={color.id} className="relative">
              ...
              <m.button
                ...
                className="relative w-full flex items-center justify-center gap-1 sm:gap-2 py-2 px-2 sm:px-3 rounded-md z-10"
                whileTap={{ scale: 0.98 }}
              >
              <AnimatePresence mode="wait">
                {color.icon ? (
                  ...
                  >
                    <m.div
                      className={`w-5 h-5 rounded-full ${color.color} border-2 ${color.border}`}
                      animate={{
                        scale: isHovering === color.id ? 1.2 : value === color.id ? 1.1 : 1,
