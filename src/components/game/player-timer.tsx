...
import { type TimeState, formatTime, getTimeWarningLevel } from '~/lib/time-control-types';
...
export function PlayerTimer({ timeState, color, className }: PlayerTimerProps) {
  ...
  const getTimerStyle = () => {
    if (timeWarning?.level === 'urgent') return 'text-red-600 font-bold animate-pulse';
    ...
    if (isTimerActive) return color === 'red' ? 'text-red-600 font-semibold' : 'text-gray-800 font-semibold';
    return 'text-gray-600';
  };

  return (
    <div className={cn('flex items-center gap-1', className)}>
      <Clock className={cn('w-3.5 h-3.5', getTimerStyle())} />
