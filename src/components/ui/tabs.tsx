...
const UnderlineTabsContext = React.createContext<UnderlineTabsContextValue | null>(null)
...
>(({ className, value, forceMount, ...props }, ref) => (
  <TabsPrimitive.Content value={value} forceMount={forceMount ? true : undefined} asChild>
...
function TabsUnderline({
  ...
  return (
    <UnderlineTabsContext.Provider
      value={{
        activeValue: (value as string | undefined) ?? internalValue,
...
function TabsUnderlineList({
  ...
  return (
    <TabsPrimitive.List
      ...
      className={cn(
        "relative flex items-end gap-0 w-full px-4",
        className
      )}
      {...props}
    >
...
>(({ className, children, value, ...props }, ref) => {
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        "relative flex-1 text-center text-sm font-semibold uppercase tracking-wide pb-3",
