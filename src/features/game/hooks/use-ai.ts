...
export function useAI() {
  ...
  const engine = useMemo(() => new CheckersAI({ difficulty: 'medium' }, state.rules), [state.rules]);
  ...
  useEffect(() => {
    ...
    const think = async () => {
      ...
      const aiMove = await engine.getBestMove(state.board, aiColor, state.moveCount);
      if (cancelled || !aiMove) {
      ...
      dispatch({ type: 'APPLY_MOVE', payload: { newBoard, move: aiMove, winner } });
      dispatch({ type: 'SET_AI_THINKING', payload: false });
    };
    void think();
  ...
  }, [state.board, state.currentPlayer, state.gameMode, state.winner, state.isViewingHistory, state.isReviewMode, state.aiDifficulty, state.moveCount, state.rules, state.playerColor, state.drawState, state.isAIThinking, dispatch, engine]);
}
