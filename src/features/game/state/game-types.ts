import type { Board as BoardType, Position, PieceColor, Move } from '~/lib/game-logic';
...
export type GameAction =
  ...
  | { type: 'APPLY_MOVE'; payload: { newBoard: BoardType; move: Move; winner: PieceColor | DrawResult | null } }
  ...
  | { type: 'SET_ANALYSIS'; payload: { analysis: GameAnalysis | null; isAnalyzing: boolean; progress?: number } }
  | { type: 'DIALOGS'; payload: Partial<Pick<GameState, 'showWinnerDialog' | 'showContinueDialog' | 'showKeyboardHelp' | 'showTimeControlDialog' | 'showDrawDialog'>> }
