...
import { createMockUser, createMockPrismaClient, createMockResendClient } from '~/test/auth-utils';
...
describe('Auth Router', () => {
  ...
  describe('register', () => {
    ...
    it('should hash the password before saving', async () => {
      ...
      const isValidHash = await bcrypt.compare(validInput.password, hashedPassword);
      
      expect(isValidHash).toBe(true);
    });

    it('should throw error when email already exists', async () => {
      mockDb.user.findFirst.mockResolvedValue(createMockUser({
        email: validInput.email,
      }));
  ...
  describe('setUsername', () => {
    ...
    it('should throw error when username is taken', async () => {
      ...
      await expect(caller.setUsername({ username: 'taken' })).rejects.toThrow(TRPCError);
      await expect(caller.setUsername({ username: 'taken' })).rejects.toMatchObject({
        code: 'CONFLICT',
        message: 'Username already taken',
      });
    });
    ...
    it('should throw error when not authenticated', async () => {
      ...
      await expect(caller.setUsername({ username: 'newusername' })).rejects.toThrow(TRPCError);
      await expect(caller.setUsername({ username: 'newusername' })).rejects.toMatchObject({
        code: 'UNAUTHORIZED',
      });
    });
  });
  ...
  describe('requestPasswordReset', () => {
    it('should create reset token and send email for existing user', async () => {
      ...
      const caller = createCaller(createMockContext());
      const result = await caller.requestPasswordReset({ email: mockUser.email! });
  ...
  describe('resetPassword', () => {
    ...
    it('should successfully reset password with valid token', async () => {
      ...
      const caller = createCaller(createMockContext());
      const result = await caller.resetPassword({ token: validToken, password: newPassword });
    ...
    it('should throw error for invalid token', async () => {
      ...
      await expect(caller.resetPassword({ token: 'invalid', password: newPassword }))
    ...
    it('should throw error for expired token', async () => {
      ...
      await expect(caller.resetPassword({ token: validToken, password: newPassword }))
        .rejects.toThrow(TRPCError);
      await expect(caller.resetPassword({ token: validToken, password: newPassword }))
        .rejects.toMatchObject({
          code: 'BAD_REQUEST',
  ...
  describe('updateProfile', () => {
    ...
    it('should throw error when not authenticated', async () => {
      ...
      await expect(caller.updateProfile({ name: 'New Name' }))
        .rejects.toThrow(TRPCError);
      await expect(caller.updateProfile({ name: 'New Name' }))
        .rejects.toMatchObject({
          code: 'UNAUTHORIZED',
    ...
    it('should validate image URL format', async () => {
      ...
      await expect(caller.updateProfile({ image: 'not-a-url' }))
        .rejects.toThrow();
    });
  });
});