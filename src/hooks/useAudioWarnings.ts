...
export function useAudioWarnings({
  ...
  const lastWarningTimeRef = useRef<Record<PieceColor, number>>({ red: 0, black: 0 });
  ...
  // Play warning sound
  const playWarning = useCallback((type: keyof typeof WARNING_SOUNDS) => {
  ...
  const checkAndPlayWarnings = useCallback(() => {
    ...
    const remainingTime = activePlayer === 'red' ? timeState.redTime : timeState.blackTime;
    ...
    for (const threshold of thresholds) {
      if (remainingTime <= threshold && !playedWarningsRef.current[activePlayer].has(threshold)) {
  ...
  }, [timeState.redTime, timeState.blackTime, timeState.activePlayer, checkAndPlayWarnings]);
